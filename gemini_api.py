...
class GeminiAPI:
    ...
    def _update_model_dropdowns(self):
        ...
        if not model_list:
            return
        for selector in self.app.model_selectors.values():
            selector.configure(values=model_list)
        
        default1 = self.app.config.get("default_model_1")
        default2 = self.app.config.get("default_model_2")
        
        self.app.model_selectors[1].set(default1 if default1 in model_list else model_list[0])
        self.app.model_selectors[2].set(default2 if default2 in model_list else model_list[0])
    ...
    def _create_model_list_for_dropdown(self):
        ...
        return self.app.available_models
    ...
    def on_model_change(self, chat_id):
        if messagebox.askyesno("Confirm", f"Changing model for Gemini {chat_id} will reset its history. Continue?"):
            self.app.config[f"default_model_{chat_id}"] = self.app.model_selectors[chat_id].get(); self.app.config_manager._save_config_to_file(self.app.config)
            self.prime_chat_session(chat_id, from_event=True)
        ...
        try:
            ...
            for chunk in response:
                if self.app.current_generation_id[chat_id] != generation_id:
                    response.resolve() # Resolve the response before returning
                    return # Interruption point
                ...
                if response is not None:
                    ...
                    if not is_ok:
                        try:
                            ...
                            self.app.response_queue.put({'type': 'error', 'chat_id': chat_id, 'text': error_text, 'generation_id': generation_id})
                else: self.app.response_queue.put({'type': 'error', 'chat_id': chat_id, 'text': "--- API call failed to return a response object. Check connection. ---", 'generation_id': generation_id})
